...
int AMRStructure::write_particles_to_file(bool pre_remesh) {
    ...
    std::ofstream q_plus_file;
    std::ofstream q_minus_file;
    ...
    xs_file.open(sim_dir + "simulation_output/xs/xs_" + remesh_str + std::to_string(iter_num), std::ios::out | std::ios::binary); 
    ...
    if (!xs_file | !ys_file | !w0s_file | !j0s_file | !q_plus_file | !q_minus_file | !u1s_file | !u2s_file | !b1s_file | !b2s_file) {
    ...
    for (int ii = 0; ii < xs.size(); ++ii) {
        ...
        double q_p0 = q_plus[ii];
        double q_m0 = q_minus[ii];
        double u1 = u1s[ii];
        double u2 = u2s[ii];
        double b1 = b1s[ii];
        double b2 = b2s[ii];
    ...
    if (!xs_file.good() | !ys_file.good() | !w0s_file.good() | !j0s_file.good() 
        | !q_plus_file.good() |!q_minus_file.good() | !u1s_file.good() | !u2s_file.good() | !b1s_file.good() | !b2s_file.good()) {
...
std::ostream& operator<<(std::ostream& os, const AMRStructure& amr) {
    ...
    os << "-----------------" << endl;
    os << "Initial height: " << amr.initial_height << ", y height: " << amr.y_height << ", height: " << amr.height << ", max height: " << amr.max_height << endl;
